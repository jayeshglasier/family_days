...
class ActiveRewardExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        $userRecord = DB::table('users')->select('id','use_is_admin','use_fam_unique_id','use_family_id')->where('use_family_id',$datarecords['familyId'])->first();
            ...
            return Rewards::select('red_rewards_name','red_brand_name','red_cat_name','red_point','use_full_name','bds_link',DB::raw("DATE_FORMAT(red_frame_date, '%b-%d-%Y') as expired_date"),DB::raw("DATE_FORMAT(red_createat, '%b-%d-%Y') as created_date"))->join('users','tbl_rewards.red_child_id','users.id')
