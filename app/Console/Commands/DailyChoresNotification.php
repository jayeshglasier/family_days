...
class DailyChoresNotification extends Command
{
    ...
    public function handle()
    {
        ...
        $recordReward = Chores::select('id as user_id','cho_id','cho_title','use_fcm_token','cho_date','use_username','cho_last_date')->join('users','tbl_chores_list.cho_child_id','=','users.id')->whereDate('cho_date',$current_date)->orderby('cho_date','ASC')->get();
        ...
        foreach ($recordReward as $key => $value) {
            ...
            $insertMessage->save();
            $this->notificationDailyChores($value['use_fcm_token'],$value->cho_title,$content,$type,$notificationId);
         }

        // -------------------- End Send notification remaining 24 hours (Chores) -------------------
    }

    public function notificationDailyChores($token, $choresTitle,$content,$type,$notificationId)
    {
